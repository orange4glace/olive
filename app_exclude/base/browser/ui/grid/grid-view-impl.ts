...
import { Event, Emitter, Relay, map, any, None } from "base/common/event";
import { LayoutPriority, SplitView, Sizing, Split } from "base/browser/ui/split-view/split-view";
import { Orientation, Sash } from "base/browser/ui/sash/sash";
import { tail2 } from "base/common/arrays";
import { IBranchNode, ILeafNode, GridNode, GridBranchNode, IView, Box, IGridViewStyles, orthogonal, IGridViewOptions, IGridView } from "base/browser/ui/grid/grid-view";
import { BranchNodeView, LeafNodeView } from "base/browser/ui/grid/grid-view-view";
...
class BranchNode implements IBranchNode {
	...
	constructor(
		...
		this.splitview = new SplitView({ orientation, styles, proportionalLayout });
		this.splitview.layout(size);

		const onDidSashReset = map(this.splitview.onDidSashReset, i => [i]);
	...
	private onDidChildrenChange(): void {
		const onDidChildrenChange = map(any(...this.children.map(c => c.onDidChange)), () => undefined);
		this.childrenChangeDisposable.dispose();
		this.childrenChangeDisposable = onDidChildrenChange(this._onDidChange.fire, this._onDidChange);

		const onDidChildrenSashReset = any(...this.children.map((c, i) => map(c.onDidSashReset, location => [i, ...location])));
	...
	render(): React.ReactNode {
		return React.createElement(BranchNodeView, {
			node: this
		});
	}
...
class LeafNode implements ILeafNode {
	...
	readonly onDidSashReset: Event<number[]> = None;
	...
	set linkedWidthNode(node: LeafNode | undefined) {
		this._onDidLinkedWidthNodeChange.input = node ? node._onDidViewChange : None;
	...
	set linkedHeightNode(node: LeafNode | undefined) {
		this._onDidLinkedHeightNodeChange.input = node ? node._onDidViewChange : None;
	...
	constructor(
		...
		this._onDidViewChange = map(this.view.onDidChange, e => e && (this.orientation === Orientation.VERTICAL ? e.width : e.height));
		this.onDidChange = any(this._onDidViewChange, this._onDidSetLinkedNode.event, this._onDidLinkedWidthNodeChange.event, this._onDidLinkedHeightNodeChange.event);
	}
	...
	render(): React.ReactNode {
		return React.createElement(LeafNodeView, this);
	}
...
export class GridView implements IGridView {
	...
	private set root(root: BranchNode) {
		...
		this._onDidChange.input = map(root.onDidChange, () => undefined); // TODO
	}

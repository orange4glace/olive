...
import { orthogonal, IView, IGridView, Sizing as GridViewSizing, Box, IGridViewStyles } from './grid-view';
import { Event, Emitter, filter, any } from 'base/common/event';
import { Split, Distribute, LayoutPriority } from 'base/browser/ui/split-view/split-view';
import { GridView } from 'base/browser/ui/grid/grid-view-impl';
...
export class Grid<T extends IView> implements IDisposable {
	...
	private views = new Map<T, React.RefObject<HTMLDivElement>>();
	...
	removeView(view: T, sizing?: Sizing): void {
		...
		this.gridview.removeView(location, sizing === Sizing.Distribute ? Distribute : undefined);
	...
	private getViewLocation(view: T): number[] {
		const element = this.views.get(view).current;

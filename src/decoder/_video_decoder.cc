...
NAPI_DEFINE_CLASS(VideoDecoder,
    ...
    NAPI_PROPERTY_FUNCTION("Seek", NAPI_Seek, napi_default))

napi_value VideoDecoder::_NAPI_Decode(int64_t timestamp) {
  return napi_encoder<int32_t>::encode(Decode(timestamp));
}

napi_value VideoDecoder::_NAPI_Seek(int64_t timestamp) {
  return napi_encoder<int32_t>::encode(Seek(timestamp));
}
